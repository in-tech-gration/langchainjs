 ...
 * @param documentPrompt Prompt used for formatting each document into a string. Input
    ...
    other inputs variables will be automatically retrieved from the `Document.metadata` object. Default to a prompt that only contains `Document.page_content`.
 * @param documentSeparator String separator to use between formatted document strings.
 * @returns An LCEL `Runnable` chain.
    Expects a object as input with a list of `Document`s being passed under
    the "context" key.
